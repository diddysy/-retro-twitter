...
export default function RetroTwitter() {
  ...
  const [view, setView] = useState("home"); // home | profile | friends
  ...
  async function fetchTweets() {
    let { data } = await supabase
      .from("tweets")
      .select("*, replies(*), votes(type), user_id")
  ...
  return (
    <div style={{ fontFamily: "Verdana", background: "linear-gradient(to right, #ffecd2, #fcb69f)", color: "black", minHeight: "100vh" }}>
      {/* Navbar */}
      <nav style={{ background: "#3399ff", padding: "15px", display: "flex", gap: "30px", fontSize: "20px" }}>
        <button onClick={() => setView("home")} style={{ fontSize: "24px" }}>🏠 Home</button>
        <button onClick={() => setView("profile")} style={{ fontSize: "24px" }}>👤 Profile</button>
        <button onClick={() => setView("friends")} style={{ fontSize: "24px" }}>👥 Friends</button>
      </nav>
      ...
      {view === "home" && (
        <>
          ...
          <div style={{ padding: "20px", background: "#fff", margin: "20px", border: "2px solid #ccc" }}>
            ...
            />
            <button onClick={postTweet} style={{ marginTop: "10px", background: "#3399ff", color: "white", padding: "10px 20px", fontSize: "18px" }}>Post</button>
          </div>

          {/* Tweets */}
          <div style={{ padding: "20px" }}>
            {tweets.map((t) => (
              <div key={t.id} style={{ border: "1px solid #999", marginBottom: "15px", padding: "15px", background: "white", borderRadius: "8px" }}>
                <p style={{ fontSize: "18px" }}>{t.content}</p>
                {t.gif_url && <img src={t.gif_url} alt="gif" style={{ maxWidth: "250px", borderRadius: "5px" }} />}
                <div style={{ display: "flex", gap: "20px", marginTop: "10px" }}>
                  <button onClick={() => voteTweet(t.id, "up")} style={{ fontSize: "20px" }}>👍 {t.votes?.filter(v=>v.type==='up').length || 0}</button>
                  <button onClick={() => voteTweet(t.id, "down")} style={{ fontSize: "20px" }}>👎 {t.votes?.filter(v=>v.type==='down').length || 0}</button>
                </div>
      ...
      {view === "profile" && (
        <div style={{ padding: "20px" }}>
          <h2>👤 My Profile</h2>
          {tweets.filter(t => t.user_id === user.id).map((t) => (
            <div key={t.id} style={{ border: "1px solid #999", marginBottom: "15px", padding: "15px", background: "white" }}>
              <p>{t.content}</p>
              {t.gif_url && <img src={t.gif_url} alt="gif" style={{ maxWidth: "250px" }} />}
            </div>
          ))}
        </div>
      )}
